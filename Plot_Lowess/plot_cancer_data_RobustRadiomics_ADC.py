...
def plot_all(feature=None):
    ...
    dfx_adc3 = pd.read_csv('RobustRadiomics_ADC_Scan3_serial.csv')
    MRI3 = dfx_adc3[feature]
    MRI3 = np.array(MRI3)
    ...
    plt.legend(loc='best')
    # plt.show()
    plt.savefig(plot_save + feature + '_ADC_negative.png')
    plt.close()
    ...
    plt.legend(loc='best')
    # plt.show()
    plt.savefig(plot_save + feature + '_ADC_positive.png')
    plt.close()

dfx_adc0= pd.read_csv('RobustRadiomics_ADC_Final_serial.csv', index_col=0, nrows=0).columns.tolist()

# featureName = dfx_adc0.head(1)
# data1 = dfx_adc0.iloc[0, 0:28]
# featureName = featureName[1, 1:]
featureName = np.array(dfx_adc0)
