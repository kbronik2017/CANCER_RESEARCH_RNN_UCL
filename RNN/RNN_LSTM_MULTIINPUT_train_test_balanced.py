...
def build_model_LSTM(allow_cudnn_kernel=True):
    ...
    lstm_layer1 = keras.layers.LSTM(200, return_sequences=True)(in1)
    lstm_layer2 = keras.layers.LSTM(200, return_sequences=True)(in2)

    merged = keras.layers.concatenate([lstm_layer1, lstm_layer2], axis=-1)
    # 64
    layer = keras.layers.LSTM(400, return_sequences=True)(merged)
    # layer = keras.layers.GRU(64, return_sequences=True)(layer)
    # 32
    layer = keras.layers.LSTM(200, return_sequences=True)(layer)
    # layer = keras.layers.GRU(32, return_sequences=True)(layer)
    # 16
    layer = keras.layers.LSTM(100, return_sequences=True)(layer)
    layer = keras.layers.LSTM(50, return_sequences=True)(layer)
    layer = keras.layers.LSTM(25, return_sequences=False)(layer)
    layer = keras.layers.BatchNormalization()(layer)

    # output = keras.layers.Dense(output_size, activation='sigmoid')(layer)
    layer = keras.layers.Dense(25, activation=None)(layer)
...
model = build_model_LSTM(allow_cudnn_kernel=True)
...
model_weights = THIS_PATH + '/MODEL/' + 'model_weights_LSTM_balanced.hdf5'
