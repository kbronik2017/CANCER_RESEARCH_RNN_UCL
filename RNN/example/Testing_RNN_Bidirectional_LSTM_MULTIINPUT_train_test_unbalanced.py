...
def build_model_Bidirectional_LSTM(allow_cudnn_kernel=True):
    ...
    lstm_layer1 = keras.layers.Bidirectional(keras.layers.LSTM(200, return_sequences=True))(in1)
    lstm_layer2 = keras.layers.Bidirectional(keras.layers.LSTM(200, return_sequences=True))(in2)

    merged = keras.layers.concatenate([lstm_layer1, lstm_layer2], axis=-1)
    # 64
    layer = keras.layers.Bidirectional(keras.layers.LSTM(400, return_sequences=True,  dropout=0.25))(merged)
    # layer = keras.layers.GRU(64, return_sequences=True)(layer)
    # 32
    layer = keras.layers.Bidirectional(keras.layers.LSTM(200, return_sequences=True,  dropout=0.25))(layer)
    # layer = keras.layers.GRU(32, return_sequences=True)(layer)
    # 16
    layer = keras.layers.Bidirectional(keras.layers.LSTM(100, return_sequences=True,  dropout=0.25))(layer)
    layer = keras.layers.Bidirectional(keras.layers.LSTM(50, return_sequences=True,  dropout=0.25))(layer)
    layer = keras.layers.Bidirectional(keras.layers.LSTM(40, return_sequences=True,  dropout=0.25))(layer)
    layer = keras.layers.Bidirectional(keras.layers.LSTM(30, return_sequences=True,  dropout=0.25))(layer)
    layer = keras.layers.Bidirectional(keras.layers.LSTM(20, return_sequences=True,  dropout=0.25))(layer)
    layer = keras.layers.Bidirectional(keras.layers.LSTM(10, return_sequences=False,  dropout=0.25))(layer)
    ...
    return model
    # lstm_layer1 = keras.layers.Bidirectional(keras.layers.LSTM(32, return_sequences=True))(in1)
...
model = build_model_Bidirectional_LSTM(allow_cudnn_kernel=True)
...
net_weights = os.path.join(THIS_PATH, 'already_trained_model', 'model_weights_Bidirectional_LSTM_unbalanced.hdf5')
model.load_weights(net_weights, by_name=True)

# sys.path.append(os.path.join(THIS_PATH, 'MODEL'))
# model_weights = THIS_PATH + '/MODEL/' + 'model_weights_Bidirectional_LSTM_unbalanced.hdf5'
