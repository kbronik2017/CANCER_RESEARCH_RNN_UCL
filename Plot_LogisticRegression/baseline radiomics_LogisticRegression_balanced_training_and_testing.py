...
finalX = np.concatenate((X1, X2), axis=1)
# for i in range(len(X1)):
#     print('X1[i]', X1[i])
#     print('X2[i]', X2[i])
#     finalX.append(np.concatenate(X1[i], X2[i]))



# finalX = sklearn.preprocessing.StandardScaler(finalX)
# print('finalX[0] ', finalX)
# print('finalX_shape ', np.array(finalX).shape)
...
def predict_print(model):
    ...
    for i in range(len(Testx)):
        ...
        result = model.predict_proba(t1)
        # print(' result',  result)
        all_predictions.append(result[0][1])

        print(
            "Predicted result: is: %.3f, Target result is: %.3f" %(result[0][1], Testy[i]))
...
predicted_results = predict_print(model=clf)

# print('PatientName', PatientName0_label[38:])
# print('PatientName', len(PatientName0_label[38:]))
rescaled1 = np.array(PatientName0_label[38:])
# print('predicted_results', predicted_results)
print('PatientNames:', rescaled1)
import xlsxwriter
workbook = xlsxwriter.Workbook('baseline radiomics_LogisticRegression_balanced.xlsx')
...
# show the plot
# plt.show()
plt.savefig('baseline radiomics_LogisticRegression_balanced.png')
